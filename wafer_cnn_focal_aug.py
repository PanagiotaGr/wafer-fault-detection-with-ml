...
def plot_class_distribution(df):
    ...
    plt.ylabel("Count")
    plt.tight_layout()
    plt.savefig(os.path.join(FIG_DIR, "focal_aug_class_distribution.png"), dpi=200)
    plt.close()
...
def augment_wafer(img):
    """
    Απλό augmentation με wafer-safe transforms:
    rotations + flips + μικρό noise
    """
    out = img.copy()
...
def plot_training_curves(history):
    ...
    plt.xlabel("Epoch")
    plt.ylabel("Loss")
    plt.title("Focal Loss Training / Validation Loss")
    plt.legend()
    plt.tight_layout()
    plt.savefig(os.path.join(FIG_DIR, "focal_aug_loss_curve.png"), dpi=200)
    plt.close()
    ...
    plt.xlabel("Epoch")
    plt.ylabel("Accuracy")
    plt.title("Focal Loss Training / Validation Accuracy")
    plt.legend()
    plt.tight_layout()
    plt.savefig(os.path.join(FIG_DIR, "focal_aug_accuracy_curve.png"), dpi=200)
    plt.close()
...
def save_confusion_matrix(cm, classes):
    ...
    plt.ylabel("True")
    plt.tight_layout()
    plt.savefig(os.path.join(FIG_DIR, "focal_aug_confusion_matrix.png"), dpi=200)
    plt.close()
...
def main():
    ...
    best_val_acc = 0.0
    best_model_path = os.path.join(OUTPUT_DIR, "best_wafer_cnn_focal_aug.pt")
    ...
    with open(os.path.join(RESULTS_DIR, "focal_aug_classification_report.txt"), "w", encoding="utf-8") as f:
    ...
    np.savetxt(
        os.path.join(RESULTS_DIR, "focal_aug_confusion_matrix.csv"),
        cm,
        delimiter=",",
        fmt="%d"
    )

    save_confusion_matrix(cm, le.classes_)

    print("\n[INFO] Focal Loss + Augmentation pipeline completed successfully.")
